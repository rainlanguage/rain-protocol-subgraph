...
import { ExpressionStateConfig  , ExpressionDeployer } from "../../generated/schema"; 
...
export function handleDeployExpression(event: DeployExpression): void {  
    ...
    if(expressionDeployer){
        deployExpressionObject.decompiledSources = decompileStateConfig( expressionDeployer.functionPointers, event)
...
function decompileStateConfig( functionPointers: string , event: DeployExpression): Bytes[] { 
   ...
   for(let i = 0 ; i < decompiledSources.length ; i++){
        ...
        log.info('decompiledSourcesBytes {} : ',  [ Bytes.fromHexString(decompiledSources[i]).toString() ] )   
   }
