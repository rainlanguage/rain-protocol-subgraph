import { Implementation, NewChild } from "../../generated/ERC20TransferTierFactory/ERC20TransferTierFactory";
...
export function handleImplementation(event: Implementation): void {
    let erc20TransferTierFactory = new ERC20TransferTierFactory(event.address.toHex())
...
export function handleNewChild(event: NewChild): void {
    let erc20TransferTierFactory = ERC20TransferTierFactory.load(event.address.toHex())
    
    let erc20TransferTierContract = ERC20TransferTierContract.bind(event.params.child)
    ...
    log.info("erc20TransferTierFactory : {}", [erc20TransferTierFactory.id])
    let children = erc20TransferTierFactory.children
    children.push(erc20TransferTier.id)
