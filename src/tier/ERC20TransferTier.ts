...
export function handleInitialize(event: Initialize): void {
  ...
  erc20.save();
  if (erc20TransferTier) {
    erc20TransferTier.token = erc20.id;

    erc20TransferTier.tierLevels = [
      getTierLevel(event.address.toHex(), BigInt.fromI32(0)),
...
export function handleTierChange(event: TierChangeEvent): void {
  ...
  let tierChange = new TierChange(
    event.transaction.hash.toHex() + " - " + event.address.toHex()
  );
  ...
  if (erc20TransferTier) tierChange.tierContract = erc20TransferTier.id;
  ...
  if (erc20TransferTier) {
    ...
    if (tierChanges) tierChanges.push(tierChange.id);
    erc20TransferTier.tierChanges = tierChanges;

    let newTierLevel = TierLevel.load(
      event.address.toHex() + " - " + event.params.endTier.toString()
    );

    if (newTierLevel) {
      newTierLevel.memberCount = newTierLevel.memberCount.plus(ONE_BI);
      newTierLevel.save();
    }

    if (event.params.startTier.notEqual(ZERO_BI)) {
      let oldTierLevel = TierLevel.load(
        event.address.toHex() + " - " + event.params.startTier.toString()
      );

      if (oldTierLevel) {
        oldTierLevel.memberCount = oldTierLevel.memberCount.minus(ONE_BI);
        oldTierLevel.save();
      }
    }
