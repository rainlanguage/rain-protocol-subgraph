...
export function handleNewChild(event: NewChild): void {
    let erc721BalanceTierFactory = ERC721BalanceTierFactory.load(event.address.toHex())

    let erc721BalanceTier = new ERC721BalanceTier(event.params.child.toHex())
    let erc20BalanceTierContract = ERC721BalanceTierContarct.bind(event.params.child)
...
export function handleImplementation(event: Implementation): void {
    let erc20BalanceTierFactory = new ERC721BalanceTierFactory(event.address.toHex())
