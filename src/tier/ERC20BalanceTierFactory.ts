...
export function handleNewChild(event: NewChild): void {
  let erc20BalanceTierFactory = ERC20BalanceTierFactory.load(
    event.address.toHex()
  );

  let erc20BalanceTier = new ERC20BalanceTier(event.params.child.toHex());
  let erc20BalanceTierContract = ERC20BalanceTierContarct.bind(
    event.params.child
  );
  ...
  if (erc20BalanceTierFactory) {
    let children = erc20BalanceTierFactory.children;
    if (children) children.push(erc20BalanceTier.id);
...
export function handleImplementation(event: Implementation): void {
  let erc20BalanceTierFactory = new ERC20BalanceTierFactory(
    event.address.toHex()
  );
