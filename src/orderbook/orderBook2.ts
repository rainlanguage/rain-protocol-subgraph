...
export function handleAfterClear(event: AfterClear): void { 
    ...
    let orderClearStateChange = new OrderClearStateChange(
      ...
      if (bounty) {
        ...
        bounty.bountyAmountB = event.params.stateChange.bOutput.minus(
    ...
    if (order_) {   
        if(order_.validInputs){ 
           let orderValidInputs = order_.validInputs  
               if(orderValidInputs){
                   let orderInputIO = orderValidInputs[event.params.takeOrder.inputIOIndex.toI32()] 
                   if(orderInputIO){ 
        ...
        if(order_.validOutputs){ 
           let orderValidOutputs = order_.validOutputs  
               if(orderValidOutputs){
                   let orderOutputIO = orderValidOutputs[event.params.takeOrder.outputIOIndex.toI32()] 
                   if(orderOutputIO){ 
...
function getOrderAdd(event: AddOrder): Order {    
  ...
  if(!order){
      ...
      order.expiresAfter = event.params.order.expiresAfter 
      ...
      for(let i = 0 ; i < validInputsParams.length ; i++){ 
          ...
          let ipTokenVault = getTokenVault(ipToken.id ,  event.params.sender.toHex() , input.vaultId) 
      ...
      for(let i = 0 ; i < validOutputParams.length ;i++){ 
          ...
          let opTokenVault = getTokenVault(opToken.id ,  event.params.sender.toHex() , output.vaultId)
...
let tupleArray_a_: Array<ethereum.Value> = [
  ...
  ethereum.Value.fromUnsignedBigInt(event.params.a.expiresAfter),
...
let tupleArray_b_: Array<ethereum.Value> = [
  ...
  ethereum.Value.fromUnsignedBigInt(event.params.b.expiresAfter),
...
function getTakeOrder(event: TakeOrder): Order[] { 
  ...
  let tupleArray_a_: Array<ethereum.Value> = [
      ...
      ethereum.Value.fromUnsignedBigInt(order_.expiresAfter),
      ethereum.Value.fromTupleArray(ipAIOTulpe),
      ethereum.Value.fromTupleArray(opAIOTulpe) 
    ];
